    ...
    <div className="service-page">
      <header className="service-header">
        <div className="logo" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          />
          <span className="head-name" style={{ fontSize: '2.5rem', fontWeight: 'bold', fontFamily: "'Vollkorn', serif" }}>
            CampusFix
          </span>
        </div>
        <button className="logout-btn btn-primary" onClick={handleLogout}>Logout</button>
            ...
            <ul className="pending-list">
              {pendingRequests.map((req) => (
                <li key={req._id} className="pending-item">
                  ...
                  <p><strong>Problem:</strong> {req.description}</p>
                  <p><strong>Service:</strong> {req.serviceType}</p>
                  <button className="action-btn" onClick={() => handleAccept(req._id)}>Accept</button>
                </li>
              ))}
            </ul>
          )}
            ...
            <ul className="history-list">
              {historyRequests.map((req) => (
                <li key={req._id} className="history-item">
                  ...
                  {req.status === 'Processing' && (
                    <button className="action-btn" onClick={() => handleMarkSuccessful(req._id)}>Mark as Successful</button>
                  )}
                </li>
              ))}
            </ul>
